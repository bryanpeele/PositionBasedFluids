...
public class ParticleSystemBuilder implements GLEventListener
{
    ...
    class BuilderGUI implements MouseListener, MouseMotionListener, KeyListener
    {
        ...
        BuilderGUI()
        {
            ...
            AbstractButton[] buttons      = { new JButton("Reset"),
                ...
                new JButton ("Add Force +X"),
                new JButton ("Add Force -X"),
                new JButton ("Add Force +Y"),
                new JButton ("Add Force -Y"),
                new JButton ("Add Force +Z"),
                new JButton ("Add Force -Z"),
            };
        ...
        class TaskSelector implements ActionListener
        {
            ...
            public void actionPerformed(ActionEvent e)
            {
                ...
                if(cmd.equals("Reset")) {
                    ...
                    loadFrameFromFile();
                } else if(cmd.equals("Add Force +X")) {
                	addForcePosX();
                } else if(cmd.equals("Add Force -X")) {
                	addForceNegX();
                } else if(cmd.equals("Add Force +Y")) {
                	addForcePosY();
                } else if(cmd.equals("Add Force -Y")) {
                	addForceNegY();
                } else if(cmd.equals("Add Force +Z")) {
                	addForcePosZ();
                } else if(cmd.equals("Add Force -Z")) {
                	addForceNegZ();
                } else {
    ...
    /**
     * Functions designed to add force equally to all particles before or during simulation
     */
    private void addForcePosX() {
    	for (Particle p : PS.P) {
    		p.v.x += DT*1000.0;
    	}
    }
    private void addForceNegX() {
    	for (Particle p : PS.P) {
    		p.v.x -= DT*1000.0;
    	}
    }
    private void addForcePosY() {
    	for (Particle p : PS.P) {
    		p.v.y += DT*1000.0;
    	}
    }
    private void addForceNegY() {
    	for (Particle p : PS.P) {
    		p.v.y -= DT*1000.0;
    	}
    }
    private void addForcePosZ() {
    	for (Particle p : PS.P) {
    		p.v.z += DT*1000.0;
    	}
    }
    private void addForceNegZ() {
    	for (Particle p : PS.P) {
    		p.v.z -= DT*1000.0;
    	}
    }
